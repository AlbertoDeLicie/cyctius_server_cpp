...
WebsocketSession::WebsocketSession(boost::asio::io_context& io, boost::asio::ip::tcp::socket socket, std::shared_ptr<RouterCoro> router) :
	m_io_context(io),
...
boost::asio::awaitable<void> WebsocketSession::async_read() {
	...
	std::string message = beast::buffers_to_string(m_read_buffer.data());

	co_await async_write(std::move(message));
...
boost::asio::awaitable<void> WebsocketSession::async_write(std::string message) {
	
	m_write_buffer.clear();
	boost::asio::mutable_buffer writable_area = m_write_buffer.prepare(message.length());
	boost::asio::buffer_copy(writable_area, boost::asio::buffer(message));
	m_write_buffer.commit(message.length());

	co_await m_websocket_stream.async_write(m_read_buffer.data());
}